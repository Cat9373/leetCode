...
class Solution:
    def maxActiveSectionsAfterTrade(self, s: str, queries):
        ...
        mergeLengths = self._getZeroMergeLengths(zeroGroups)
...
if __name__ == "__main__":
    ...
    queries = []
    print("Enter each query as: left right")

    for i in range(q):
        l, r = map(int, input(f"Query {i + 1}: ").split())
        queries.append([l, r])

    sol = Solution()
    result = sol.maxActiveSectionsAfterTrade(s, queries)
